...
const STORAGE={
  ...
  orderDates:"abg_v3_order_dates",medicineMeta:"abg_v3_medicine_meta"
};
...
async function loadMonthlyConsumption(file,key){
  ...
  renderAll();
  alert(`Consumo ${monthLabel(key)} guardado para ${nup} medicamentos.${replaced?`\nValores reemplazados: ${replaced}.`:""}${newValues?`\nValores nuevos: ${newValues}.`:""}`);
}
...
function renderTable(){
  ...
  $("medTable").innerHTML=rows.map(m=>{const c=cpm3(m),s=stockFor(m),d=stockDelta(m),cov=coverageMonths(m),ac=action(m);return `<tr><td><button type="button" class="medicine-link" data-med="${encodeURIComponent(m.glosaABG)}"><strong>${esc(m.glosaABG)}</strong></button>${m.controlado?'<br><span class="badge neutral">Controlado</span>':''}</td><td>${badge(program(m))}</td><td>${fmt(s)}</td><td>${d===null?"—":(d>0?"+":"")+fmt(d,0)}</td><td>${fmt(c)}</td><td>${cov===null?"—":fmt(cov,2)+" meses"}</td><td>${esc(trend(m))}</td><td>${badge(risk(m))}</td><td>${fmt(c===null?null:c*.2)}</td><td>${fmt(c===null?null:c*.4)}</td><td>${badge(state(m))}</td><td>${ac==="REVISAR BODEGA"?`<button class="secondary warehouse-btn" data-med="${encodeURIComponent(m.glosaABG)}">Revisar bodega</button>`:badge(ac)}</td><td>${esc(m.glosaBodega||"—")}</td></tr>`}).join("");
...
function exportTracerReport(){
  ...
  const rows=master.filter(m=>m.trazador).map(m=>{
    const c=cpm3(m),s=stockFor(m);
    return {
      "Fármaco":m.glosaABG,
      "Programa":program(m),
      "CPM últimos 3 meses":c,
      "20% disponibilidad":c===null?null:c*.2,
      "Stock actual":s,
      "Estado":s!==null&&c!==null&&s<c*.2?"BAJO 20%":"OK",
      "Fecha stock":todayISO()
    };
  });
...
function backup(){downloadJSON({version:VERSION,date:new Date().toISOString(),timeline,warehouse,arsenalAdd,arsenalOverride,arsenalRemoved,weeklyHistory,generalDates,generalHistory,participants,ordersHistory,expiries,stockouts,medicineMeta},`Respaldo_ABG_${todayISO()}.json`)}
