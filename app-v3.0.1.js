...
const ORDER_DATES=["2026-09-17","2026-10-16","2026-11-13","2026-12-18"];
...
function nextOrderInfo(){
  const today=new Date(todayISO()+"T00:00:00");for(const d of ORDER_DATES){const od=new Date(d+"T00:00:00"),diff=Math.round((od-today)/86400000);if(diff>=0)return{date:d,diff,eligible:diff<=3}}return null
}
...
function exportTracerReport(){const key=previousMonthKey();const missing=master.filter(m=>m.trazador&&!Object.prototype.hasOwnProperty.call(getTimeline(m),key));if(missing.length)return alert(`Faltan consumos de ${monthLabel(key)} para ${missing.length} trazadores.`);const ls=localStorage.getItem(STORAGE.lastStock);if(!sameDay(ls))return alert("Carga el stock Rayen de hoy antes de generar el reporte.");const rows=master.filter(m=>m.trazador).map(m=>{const c=cpm3(m),s=stockFor(m);return{"Fármaco":m.glosaABG,"Programa":program(m),"CPM últimos 3 meses":c,"20% disponibilidad":c===null?null:c*.2,"Stock actual":s,"Estado":s!==null&&c!==null&&s<c*.2?"BAJO 20%":"OK","Fecha stock":todayISO()}});exportWorkbook({"Trazadores":rows},`Trazadores_IAAPS_FOFAR_${todayISO()}.xlsx`)}
...
$("tracerReportBtn").onclick=exportTracerReport;$("expiriesBtn").onclick=()=>{renderExpiries();$("expiriesDialog").showModal()};$("saveExpiry").onclick=saveExpiry;$("exceptionsBtn").onclick=()=>{renderExceptions();$("exceptionsDialog").showModal()};$("exportManagementBtn").onclick=exportManagement;
